...
#define SCAN_INTERVAL 5 // describes how often a scan (sensor) will be activated
...
static u1_t count = 0; //we need to keep track of the amount of scans
...
void onEvent (ev_t ev) {
    ...
    switch(ev) {
        ...
        case EV_TXCOMPLETE:
            ...
            os_setCallback(&scanjob, do_scan); // We schedule the next scan as soon as possible after the transmission is complete
            //and pass the control of the app back to the jobs. 
...
void do_scan(osjob_t* j){
    if (count < MAX_SCANS) {
        ...
        mydata[count] = random(255); //generates a random value since I am without a sensor 
        count++; 
        ...
        os_setTimedCallback(&scanjob, os_getTime()+sec2osticks(SCAN_INTERVAL), do_scan); // sets the next scan at the required time,
        // similarly to the periodic sends in the previous example.
    } else {
        count = 0; //we only need to reinitialize the count, since we can simply write over the data in the array
